...
feature 'as a visitor' do
  context 'when visiting the /get_started' do
    it 'sees info to get started' do
      ...
      expect(page)
        .to have_content('Filter results by selecting' \
                         ' a filter on the side bar of the homepage.')
      expect(page).to have_content('Register to bookmark segments.')
      expect(page)
        .to have_content('Sign in with census if you are' \
                         ' a current student for addition content.')
